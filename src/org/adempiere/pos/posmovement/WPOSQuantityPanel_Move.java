 ...
 * 
 */
public class WPOSQuantityPanel_Move extends WPOSSubPanel_Move implements POSPanelInterface {
	...
	public WPOSQuantityPanel_Move(WPOS_Move posPanel) {
		super(posPanel);
	}
	...
	/**	Process Action 						*/
	private WPOSFilterMenu_Move actionFilterMenu;
	...
	protected void init() {
		...
		actionFilterMenu = new WPOSFilterMenu_Move(posPanel);
		...
		try {
			if (Events.ON_CTRL_KEY.equals(e.getName())) {
				...
				if (keyEvent.getKeyCode() == 114 ) {
					posPanel.deleteLine(posPanel.getM_InventoryLine_ID());
...
//				actionFilterMenu.getPopUp().setPage(this.getPage());
	...
	public void refreshPanel() {
		if(posPanel.hasLines()){
			...
			if(posPanel.getInventory().getDocStatus().compareToIgnoreCase(MOrder.STATUS_Drafted)==0 || 
					posPanel.getInventory().getDocStatus().compareToIgnoreCase(MOrder.STATUS_InProgress)==0 ){
