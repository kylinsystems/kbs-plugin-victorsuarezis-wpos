...
package org.adempiere.pos.posmovement.search;
 ...
 */
public class WQueryInventoryHistory extends WPOSQuery_Move implements POSQueryInterface
{
	/**
	 * 
	 */
	 ...
	 * 	Constructor
	 */
	public WQueryInventoryHistory (WPOS_Move posPanel)
	{
		super(posPanel);
	}	//	PosQueryProduct
	...
	private static ColumnInfo[] columnInfos = new ColumnInfo[] {
		new ColumnInfo(" ", "M_Inventory_ID", IDColumn.class),
	...
	protected void init()
	{
		setTitle(Msg.translate(Env.getCtx(), "M_Inventory_ID"));
		...
		posTable.prepareTable (columnInfos, "M_Inventory",
...
//		posTable.setHeight("99%");
	...
	public void setResults (Properties ctx, boolean processed, String doc, Date dateFrom, Date dateTo, boolean aDate)
	{
		...
		try  {
			sql.append(" SELECT inv.M_Inventory_ID, inv.DocumentNo, dt.Name AS C_DocType_ID ,")
				.append(" TRUNC(inv.movementdate,'DD') as datemovement, ")
				// priority for open amounts: invoices, allocations, order
			    .append(" inv.Processed ")
				.append(" FROM M_Inventory inv ");
			
			if (Env.isBaseLanguage(Env.getAD_Language(ctx), "C_DocType"))
				sql.append(" INNER JOIN C_DocType      dt ON (inv.C_DocType_ID = dt.C_DocType_ID)");
			else
				sql.append(" INNER JOIN C_DocType_trl  dt ON (inv.C_DocType_ID = dt.C_DocType_ID AND dt.AD_LANGUAGE = '" + Env.getAD_Language(ctx) + "')");
			
				sql.append(" WHERE  inv.DocStatus <> 'VO'")
				.append(" AND inv.C_POS_ID = ?")
				.append(" AND inv.Processed= ?");
			if (doc != null && !doc.equalsIgnoreCase(""))
				sql.append(" AND (inv.DocumentNo LIKE '%" + doc + "%')");
			if ( dateFrom != null && aDate) {
				if ( dateTo != null && !dateTo.equals(dateFrom))
					sql.append(" AND inv.movementdate >= ? AND inv.movementdate <= ?");						
				else
					sql.append(" AND inv.movementdate = ? ");	
			}
			//	Group By
			sql.append(" GROUP BY inv.M_Inventory_ID, inv.DocumentNo, dt.Name , inv.Processed");
			sql.append(" ORDER BY inv.Updated");
			int i = 1;			
	...
	protected void enableButtons()
	{
		...
		if (enabled)
		{
			...
			if (ID != null)
			{
				inventoryId = ID.intValue();
			}
		}
		logger.info("ID=" + inventoryId);
	}	//	enableButtons
	...
	{
		logger.info("M_Inventory_ID=" + inventoryId);
		
		if (inventoryId > 0)
		{
			posPanel.setInventory(inventoryId);
