 ...
 */
public class WPOSFilterMenu_Move implements  POSQueryListener, EventListener<Event>{

	private WPOS_Move pos;
	...
	public WPOSFilterMenu_Move(WPOS_Move pos)
	{
		this.pos = pos;
	...
	public void onEvent(Event actionEvent) throws Exception {
		...
		if(queryFilter.equals("All")){
			pos.getInvLinePanel().lineTableHandle.prepareTable();
			pos.getInvLinePanel().refreshPanel();
			int numItem = pos.getInvLinePanel().posTable().getItems().size();
			for (int i = 0; i < numItem; i++) {
				pos.getInvLinePanel().posTable().getItems().get(i).setStyle(null);
			}
			pos.getInvLinePanel().isFilter = false;
		}
		else{
			String whereClause = "";
...
//			}
		...
		// =====! STEP_2 utilizzo del campo 'AD_UserQuery.code' per specificare una query filtro sulla lista degli orderLine
			...
			whereClause = code;
			String sqlWhereBase = pos.getInvLinePanel().lineTableHandle.getSqlWhere();
			pos.getInvLinePanel().lineTableHandle.setSqlWhere(sqlWhereBase+" AND "+whereClause);
			pos.getInvLinePanel().lineTableHandle.prepareTable();
			pos.getInvLinePanel().refreshPanel();
			pos.getInvLinePanel().lineTableHandle.setSqlWhere(sqlWhereBase);
			
			int numItem = pos.getInvLinePanel().posTable().getItems().size();
			for (int i = 0; i < numItem; i++) {
				pos.getInvLinePanel().posTable().getItems().get(i).setStyle("background:yellow !important;");      
			}
			pos.getInvLinePanel().isFilter = true;
		// =====!!! FINE
