 ...
 */
public class WQueryReturnHistory extends WPOSQuery_Return implements POSQueryInterface
{
	/**
	 * 
	 */
	 ...
	 * 	Constructor
	 */
	public WQueryReturnHistory (WPOS_Return posPanel)
	{
		super(posPanel);
	}	//	PosQueryProduct
	...
	protected void init()
	{
		setTitle(Msg.translate(Env.getCtx(), "M_InOut_ID"));
		...
		posTable.prepareTable (columnInfos, "M_InOut",
...
//		posTable.setHeight("99%");
	...
	public void setResults (Properties ctx, boolean processed, String doc, Date dateFrom, Date dateTo, String bPartner, boolean aDate, ListItem valueSaleRep)
	{
		...
		try  {
			sql.append(" SELECT m.M_InOut_ID, m.DocumentNo, dt.Name AS C_DocType_ID, refTrl.Name AS DocStatus, au.Name,")
			...
			//
			if(posPanel.getM_POS()!=null && posPanel.getM_POS().getC_DocType_ID()>0)
				sql.append(" AND m.C_DocType_ID = "+posPanel.getM_POS().getC_DocType_ID());
			//	Group By
//			sql.append(" GROUP BY o.C_Order_ID, o.DocumentNo, au.Name, dt.Name , refTrl.Name, b.Name, o.GrandTotal, o.Processed, i.IsPaid ");
	...
	{
		logger.info("M_InOut_ID=" + returnId);
		
		if (returnId > 0)
		{
			posPanel.setReturn(returnId);
