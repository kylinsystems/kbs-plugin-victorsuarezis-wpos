 ...
 * 
 */
public class WPOSQuantityPanel_Return extends WPOSSubPanel_Return implements POSPanelInterface {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -625074695028571518L;
	
	public WPOSQuantityPanel_Return(WPOS_Return posPanel) {
		super(posPanel);
	}
	...
	*/
	...
	/**	Process Action 						*/
	private WPOSFilterMenu_Return actionFilterMenu;
...
//		}
		...
		*/
		//iDempiereConsulting __07/03/2019 -----
		actionFilterMenu = new WPOSFilterMenu_Return(posPanel);
		...
		try {
			if (Events.ON_CTRL_KEY.equals(e.getName())) {
				...
				if (keyEvent.getKeyCode() == 114 ) {
					posPanel.deleteLine(posPanel.getM_InOutLine_ID());
	...
	public void refreshPanel() {
		if(posPanel.hasLines()){
			...
			if(posPanel.getReturn().getDocStatus().compareToIgnoreCase(MInOut.STATUS_Drafted)==0 || 
					posPanel.getReturn().getDocStatus().compareToIgnoreCase(MInOut.STATUS_InProgress)==0 ){
				buttonDelete.setEnabled(true);
				buttonPlus.setEnabled(true);
