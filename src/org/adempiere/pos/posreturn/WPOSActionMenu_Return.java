...
import org.adempiere.pos.posreturn.search.WPOSQuery_Return;
 ...
 */
public class WPOSActionMenu_Return implements  POSQueryListener, EventListener<Event>,ActionListener{

    private WPOS_Return pos;
    private WPOSQuery_Return queryPartner;
    private Menupopup popupMenu;
	public static final String EVENT_ATTRIBUTE = "EVENT";
	
	private int tabID = 0; 

    public WPOSActionMenu_Return(WPOS_Return pos)
    {
        this.pos = pos;
...
//        	tabID = DB.getSQLValue(null, "SELECT AD_Tab_ID FROM AD_Tab WHERE AD_Window_ID= ?  AND TabLevel =0",table.getAD_Window_ID());
    ...
    public void okAction(POSQueryInterface query) {
        if (query.getRecord_ID() <= 0)
            return;
        //	For Ticket
        if(query instanceof WQueryBPartner_Return) {
	...
	public void actionPerformed(ActionEvent event) {
		...
		m_viewIDMap.add(new KeyNamePair(pos.getM_InOut_ID(),""));
	...
	protected ProcessInfo prepareProcess (int processId){
		 ...
		 m_pi.setRecord_ID(pos.getM_InOut_ID());
