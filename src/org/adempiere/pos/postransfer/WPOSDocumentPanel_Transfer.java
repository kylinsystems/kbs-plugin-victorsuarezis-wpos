 ...
 */
public class WPOSDocumentPanel_Transfer extends WPOSSubPanel_Transfer implements POSKeyListener, POSPanelInterface {
	 ...
	 */
	public WPOSDocumentPanel_Transfer(WPOS_Transfer posPanel) {
		super (posPanel);
	}	//	PosSubFunctionKeys
	...
	/**	Logger				*/
	private static CLogger 	log = CLogger.getCLogger(WPOSDocumentPanel_Transfer.class);
	/**	Panels				*/
...
//		v_TotalsPanel.setStyle("width:130%;height:100%");
		...
		bPartnerName.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
		bPartnerName.setWidth("100%");
		bPartnerName.addEventListener(this);
		
		btnAddInfo = new Button(" INFO ");
		btnAddInfo.setHeight("35px");
		btnAddInfo.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
...
//		row.setSpans("2");
		...
		style.setContent(".z-fieldset { margin-left:-5px }"
		    + ".z-combo-item-text { Font-family:Courier New}"
				...
				+ ".label-description {"+WPOS_Transfer.FONTSIZEMEDIUM+" display:block; height:15px; font-weight:bold; width: 415px; overflow:hidden;}"
		...
		f_lb_DocumentNo.setStyle(WPOS_Transfer.FONTSIZEMEDIUM);
		row.appendChild(f_lb_DocumentNo.rightAlign());
		
		documentNo = new Label();
		documentNo.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
		...
		documentType.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold; width:auto !important;max-width:225px !important; white-space:pre;");
		...
		f_lb_DocumentStatus.setStyle(WPOS_Transfer.FONTSIZEMEDIUM);
		row.appendChild(f_lb_DocumentStatus);
		documentStatus= new Label();
		documentStatus.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
		...
		f_lb_SalesRep.setStyle(WPOS_Transfer.FONTSIZEMEDIUM);
...
//		salesRep = new Label(posPanel.getSalesRepName());
		salesRep.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
			...
			public void onEvent(Event event) throws Exception {
				if(posPanel.hasMovement()){
					...
					if(item != null && item.getValue()!=null){
						...
						posPanel.getMovement().setSalesRep_ID(salesR_ID);
						posPanel.getMovement().saveEx();
		...
		lDocumentDate.setStyle(WPOS_Transfer.FONTSIZEMEDIUM);
		row.appendChild(lDocumentDate);
		
		documentDate = new Label();
		documentDate.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold");
		...
		lDocumentDatePromised.setStyle(WPOS_Transfer.FONTSIZEMEDIUM);
		row.appendChild(lDocumentDatePromised);
		
		documentDatePromised = new Label();
		documentDatePromised.setStyle(WPOS_Transfer.FONTSIZEMEDIUM+"; font-weight:bold; cursor: pointer; background: linear-gradient(to bottom, #f8ffe8 24%,#e3f5ab 69%,#b7df2d 92%);");
		documentDatePromised.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				if(posPanel.hasMovement()){
					...
					posPanel.getMovement().setMovementDate(dateMovement);
					posPanel.getMovement().saveEx();
					posPanel.refreshHeader();
				}
			}
...
//		}
	...
	private void findBPartner() {
		...
		MBPartnerInfo[] results = MBPartnerInfoPOS.find(ctx, value, taxId,
			contact, name, name2, eMail, phone, city, true);
