 ...
 *
 */
public class WPOS_Shopfl extends CPOS_Shopfl implements IFormController, EventListener<Event>, java.util.EventListener,
		POSPanelInterface, POSScalesPanelInterface {
	...
	/** User Pin Listener 			*/
	private WPOSUserPinListener_Shopfl		userPinListener;
	/** Electronic Scales			*/
	...
	/** Order Panel							*/
	private WPOSActionPanel_Shopfl			actionPanel;
	/** Quantity panel **/
	private WPOSQuantityPanel_Shopfl		quantityPanel;
	/**	Info Product Panel	*/
	private WPOSInfoProduct_Shopfl 			infoProductPanel;
	/** Current Line		*/
//	private WPOSProductionLinePanel			productionPanel;
	/** Function Keys		*/
	private WPOSDocumentPanel_Shopfl		documentPanel;
	/** Status Bar 							*/
...
//		settingKeyboardFocusManager();
	...
	public void init () {
		...
		try {
			...
			if(hasProduction() && actionPanel!=null){
				getMainFocus();
	...
	private boolean dynInit() { 
		setMPOS();
		userPinListener = new WPOSUserPinListener_Shopfl(this);
		...
		actionPanel = new WPOSActionPanel_Shopfl(this);
		documentPanel = new WPOSDocumentPanel_Shopfl(this);
//		productionPanel = new WPOSProductionLinePanel(this);
		infoProductPanel = new WPOSInfoProduct_Shopfl(this);
		quantityPanel = new WPOSQuantityPanel_Shopfl(this);
		East east = new East();
...
//		this.actionPanel.appendChild(productionPanel);
	...
	private void setMPOS() {
		...
		listTerminal.setStyle("height:45px;"+WPOS_Shopfl.FONTSIZEMEDIUM);
		row.setSpans("2");
...
//		actionPanel.resetPanel();
	...
	public void addOrUpdateLine(int p_M_Product_ID, BigDecimal m_QtyOrdered) {
		...
		//	
		refreshPanel();
//		productionPanel.seekFromProduct(p_M_Product_ID);
	}
	
	public WPOSInfoProduct_Shopfl getInfoProduct() {
		return infoProductPanel;
	}
	
	public WPOSQuantityPanel_Shopfl getQtyProduct() {
...
//		productionPanel.changeViewPanel();
	}
	 ...
	 * @return void
	 */
	public void newProduction(int temp) {
		newProduction();
		infoProductPanel.resetValues();
		quantityPanel.resetPanel();
		getMainFocus();
...
//		productionPanel.updateLine();
	...
	@Override
	public void moveUp() {
//		productionPanel.moveUp();
	}

	@Override
	public void moveDown() {
//		productionPanel.moveDown();
	}
	...
	public boolean isUserPinValid()
	{
		...
		if (isCorrectUserPin == null)
			WPOSUserPinDialog_Shopfl.show(this);
...
//		productionPanel.disableTable();
	}
	
	public void restoreTable(){
//		productionPanel.enableTable();
	}
	
//	public WPOSProductionLinePanel getProductionPanel(){
