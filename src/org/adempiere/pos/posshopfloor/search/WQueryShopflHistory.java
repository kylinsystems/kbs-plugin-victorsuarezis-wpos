...
package org.adempiere.pos.posshopfloor.search;
 ...
 */
public class WQueryShopflHistory extends WPOSQuery_Shopfl implements POSQueryInterface {
	 ...
	 * 	Constructor
	 */
	public WQueryShopflHistory (WPOS_Shopfl posPanel)
	{
		super(posPanel);
	}	//	PosQueryProduct
	...
	private static ColumnInfo[] columnInfos = new ColumnInfo[] {
		new ColumnInfo(" ", "LIT_ShopfloorControl_ID", IDColumn.class),
	...
	protected void init()
	{
		setTitle(Msg.translate(Env.getCtx(), "LIT_ShopfloorControl_ID"));
		...
		posTable.prepareTable (columnInfos, "LIT_ShopfloorControl",
				"C_POS_ID = " + posPanel.getC_POS_ID()
				, false, "LIT_ShopfloorControl");
...
//		posTable.setHeight("99%");
	...
	public void setResults (Properties ctx, boolean processed, String doc, Date dateFrom, Date dateTo, boolean aDate)
	{
		...
		try  {
			sql.append(" SELECT sh.LIT_ShopfloorControl_ID, sh.DocumentNo, dt.Name AS C_DocType_ID ,")
				.append(" TRUNC(sh.movementdate,'DD') as datemovement, ")
				// priority for open amounts: invoices, allocations, order
			    .append(" sh.Processed ")
				.append(" FROM LIT_ShopfloorControl sh ");
			
			if (Env.isBaseLanguage(Env.getAD_Language(ctx), "C_DocType"))
				sql.append(" INNER JOIN C_DocType      dt ON (sh.C_DocType_ID = dt.C_DocType_ID)");
			else
				sql.append(" INNER JOIN C_DocType_trl  dt ON (sh.C_DocType_ID = dt.C_DocType_ID AND dt.AD_LANGUAGE = '" + Env.getAD_Language(ctx) + "')");
			
				sql.append(" WHERE  sh.DocStatus <> 'VO'")
				.append(" AND sh.C_POS_ID = ?")
				.append(" AND sh.Processed= ?");
			if (doc != null && !doc.equalsIgnoreCase(""))
				sql.append(" AND (sh.DocumentNo LIKE '%" + doc + "%')");
			if ( dateFrom != null && aDate) {
				if ( dateTo != null && !dateTo.equals(dateFrom))
					sql.append(" AND sh.movementdate >= ? AND sh.movementdate <= ?");						
				else
					sql.append(" AND sh.movementdate = ? ");	
			}
			//	Group By
			sql.append(" GROUP BY sh.LIT_ShopfloorControl_ID, sh.DocumentNo, dt.Name , sh.Processed");
			sql.append(" ORDER BY sh.Updated");
			int i = 1;			
	...
	{
		logger.info("LIT_ShopfloorControl_ID=" + productionId);
